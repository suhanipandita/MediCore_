...
import styles from './SignupStaffPassword.module.css'; // We will create this
...
interface ValidationState { hasLetter: boolean; hasNumber: boolean; hasSpecialChar: boolean; has8Chars: boolean; }
const CheckIcon = () => (
    <svg className={styles.checkIcon} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M8 0C3.58187 0 0 3.58187 0 8C0 12.4181 3.58187 16 8 16C12.4181 16 16 12.4181 16 8C16 3.58187 12.4181 0 8 0ZM7.0625 12.1812L3.64375 8.7625L5.05625 7.35L7.0625 9.35625L10.9437 5.475L12.3562 6.8875L7.0625 12.1812Z" />
    </svg>
);
const RadioIcon = () => (
     <svg className={styles.radioIcon} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M8 1C4.13438 1 1 4.13438 1 8C1 11.8656 4.13438 15 8 15C11.8656 15 15 11.8656 15 8C15 4.13438 11.8656 1 8 1ZM8 0C12.4181 0 16 3.58187 16 8C16 12.4181 12.4181 16 8 16C3.58187 16 0 12.4181 0 8C0 3.58187 3.58187 0 8 0Z" />
    </svg>
);
const PasswordChecklist = ({ validation }: { validation: ValidationState }) => {
    return (
        <div className={styles.passwordChecklist}>
            ...
            <ul>
                <li className={validation.hasLetter ? styles.valid : ''}>{validation.hasLetter ? <CheckIcon /> : <RadioIcon />}<span>1 letter</span></li>
                <li className={validation.hasNumber ? styles.valid : ''}>{validation.hasNumber ? <CheckIcon /> : <RadioIcon />}<span>1 number</span></li>
                <li className={validation.hasSpecialChar ? styles.valid : ''}>{validation.hasSpecialChar ? <CheckIcon /> : <RadioIcon />}<span>1 Special character</span></li>
                <li className={validation.has8Chars ? styles.valid : ''}>{validation.has8Chars ? <CheckIcon /> : <RadioIcon />}<span>8 characters</span></li>
            </ul>
        </div>
    );
};
...
function SignupStaffPassword() {
    ...
    const validatePasswordStep = (): boolean => {
        ...
        if (!Object.values(validationState).every(Boolean)) {
            setPasswordError("Password does not meet all requirements."); isValid = false;
    ...
    const handleInputChange = (setter: React.Dispatch<React.SetStateAction<string>>, errorSetter: React.Dispatch<React.SetStateAction<string>>) => (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (errorSetter === setConfirmPasswordError) setConfirmPasswordError("");
    };
        ...
        <div className={styles.container}>
            ...
            <div className={styles.right}>
                <div className={styles.formContainer}>
                    <button onClick={() => navigate('/signup-staff', { state: { email }})} className={styles.backLink}>
                        <BackIcon /> Go Back
                    </button>
                    <ProgressBar currentStep={2} />
                    <h2 className={styles.title}>Create a Staff Account</h2>
                    <p className={styles.subheading}>Set a secure password for your Staff Portal.</p>
                    ...
                    <form className={styles.form} onSubmit={handlePasswordContinue} noValidate>
                        <div className={styles.inputGroup}>
                            ...
                            {isPasswordActive && passwordError && !Object.values(validationState).every(Boolean) && (
                                <div className={styles.errorContainer}> <ErrorIcon /> <p className={styles.errorText}>{passwordError}</p> </div> 
                            )}
                        </div>
                        <div className={styles.inputGroup}>
                            <div className={styles.passwordWrapper}>
                                <input
                                    id="signup-confirm-password" type={showPassword ? 'text' : 'password'}
...
export default SignupStaffPassword;