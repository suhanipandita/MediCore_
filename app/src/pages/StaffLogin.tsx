...
function StaffLogin() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Verify the user is staff (doctor or nurse)
            ...
            if (userData && (userData.role === 'Doctor' || userData.role === 'Nurse')) {
                navigate('/dashboard', { replace: true });
            } else {
                await supabase.auth.signOut();
                setGeneralError("This login is for staff members only. Please use the correct login page.");
            }
        } catch (error: any) {
    ...
    return (
        <div className={styles.container}>
            <AuthGraphic
                heading="Welcome Back, Staff"
                description="Access your medical portal to manage appointments and patient care."
            />

            <div className={styles.right}>
                <div className={styles.formContainer}>
                    <h2 className={styles.title}>Staff Login</h2>
                    <p className={styles.subheading}>Access your medical professional dashboard.</p>

                    {generalError && <p className={styles.errorGlobal}>{generalError}</p>}

                    <form className={styles.form} onSubmit={handleLogin} noValidate>
                        <div className={styles.inputGroup}>
                            <label htmlFor="staff-email" className={styles.label}></label>
                            <input
                                id="staff-email" type="email" placeholder="Email" value={email}
                        ...
                        <div className={styles.inputGroup}>
                            <label htmlFor="staff-password" className={styles.label}></label>
                            <input
                                id="staff-password" type="password" placeholder="Password" value={password}
                    ...
                    <p style={{ fontSize: '15px', fontWeight: '500px', marginTop: '20px' }}className={styles.footerText}>Not a staff member?{" "}
